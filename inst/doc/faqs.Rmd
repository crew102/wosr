...
### 4. How do I link the `author` and `address` data frames returned by `pull_wos()` together?
...
### 5. How do I download data for a query that returns more than 100,000 records?

The WoS API does not allow you to download data for a query that matches 100,000 or more publications. You can get around this by breaking your query into pieces using the publication year tag (`PY`). For example, if you have a broad query like `"TS = dog"` (which matches over 250,000 records), you could break it up into four sub-queries that have congruous date ranges (and which return fewer than 100,000 records each), then append the results together. E.g., `queries <- c("TS = dog AND PY = 1900-1980", "TS = dog AND PY = 1981-2000", "TS = dog AND PY = 2001-2010", "TS = dog AND PY = 2011-2018"); results <- lapply(queries, pull_wos)`.
